...
@register_model("wav2vec2_mixed")
class Wav2Vec2MixedModel(BaseFairseqModel):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument(
            "--loss-mask-prob", type=float, default=0.5)
    ...
    def __init__(self, args):
        ...
        self.loss_mask_prob = args.loss_mask_prob
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False):
        ...
        if mask:
            ...
            pdb.set_trace()
            loss_mask = torch.empty(mask_indices.size()).fill_(self.loss_mask_prob)
            loss_mask = torch.bernoulli(loss_mask).type_as(mask_indices)
            loss_mask = loss_mask | mask_indices
            if loss_mask is not None:
                y = unmasked_features[loss_mask].view(
        ...
        x = x[loss_mask].view(x.size(0), -1, x.size(-1))
...
@register_model_architecture("wav2vec2_mixed", "wav2vec2_mixed")
